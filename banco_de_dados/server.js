  ...
  try {
    ...
    const [result] = await pool.execute(`
      INSERT INTO game_results
        (user_id, game_id, title, score, total_questions, correct_answers, completed, medal, feedback)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      req.user.id,
      gameId,
    ...
    console.error(error);
    return res.status(500).json({ message: 'Erro ao salvar resultado do jogo.' });
  }
});
